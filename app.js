...
const CalandarData = function (totalDays, firstDay) {
    const defaultWeekDays = [0, 0, 0, 0, 0, 0, 0];
...
const CalanderUI = function (calandar) {
    const weekDaysMarkup = days.map(
        (day) => {
          return  `<div class="day-use"><strong> ${day.slice(0, 3).toUpperCase()} </strong></div> `
    ...
    const weeksMarkup = calandar.map((week) => {
        const daysMarkup = week.map((day) => {
            return `<div id="day-${day}" class="day-use"> ${day ? `<span> ${day}</span>` : ""
                } </div>`;
        });
        return `
                    <div class="week-row">
...
const changeCalanderMonthYear = function (month, year) {
    ...
    const calander = CalandarData(totalDaysInMonth, firstDayInMonth);
